...
from src.strategy_swing import should_enter, sizing, should_exit


def main():
    print("[run_swing] Started")
    while True:
        analyses = analyze_clusters(max_items=10, contradiction_threshold=2, ignore_contradiction=False)
        open_pos = get_open_positions(strategy="swing")
        ...
        for pos in open_pos:
            ...
            if current > 0:
                ...
                if reason:
                    close_position(pos["id"], current, f"swing_{reason}")
                    print(f"[swing] Closed {proto} at {current} ({reason})")
        ...
        for a in analyses:
            ...
            if should_enter(a):
                ...
                if current > 0:
                    ...
                    print(f"[swing] Opened {proto} at {current} (id={pos_id})")
